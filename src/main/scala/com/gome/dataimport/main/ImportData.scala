...
object ImportData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // schema 文件读取
    val sch: DataFrame = sqlContext.csvFile(args(0), useHeader = true, quote = '\'')
    val schema: StructType = sch.schema
    val columns: Seq[String] = schema.map(s => s.name).filter(name => !name.toUpperCase.equals("HASHED"))
    val map = Map[String, List[String]](args(3) -> columns.toList)

    // 数据文件读取
    val frame: DataFrame = sqlContext.csvFile(args(1) + ".format", useHeader = false, quote = '\'', ignoreLeadingWhiteSpace = true, ignoreTrailingWhiteSpace = true)
    val frame1: DataFrame = sqlContext.createDataFrame(frame.map(r => r), schema)
    frame1.registerTempTable("tempTable")

    //生成最终DF
    val results: DataFrame = sqlContext.sql("select " + args(4) + " as key," + columns.mkString(",") + " from tempTable")
